...
for ix, i in df.iterrows():
    ...
    if last_value > 45_000_000:
        if curr_value > last_value:
            df.loc[ix, "execution_payload_gas_limit_adj"] = 60_000_000
        elif curr_value < last_value:
            df.loc[ix, "execution_payload_gas_limit_adj"] = 45_000_000
        else:
            df.loc[ix, "execution_payload_gas_limit_adj"] = 45_000_000
    else:
        if curr_value > last_value:
            if int(i["proposer_index"]) in reg_keys:
                if reg[int(i["proposer_index"])] >= 60_000_000:
                    df.loc[ix, "execution_payload_gas_limit_adj"] = 60_000_000
                else:
                    df.loc[ix, "execution_payload_gas_limit_adj"] = 45_000_000
            else:
...
#df["execution_payload_gas_limit_adj"] = df["execution_payload_gas_limit"].apply(lambda x: 30_000_000 if x <= 30_000_000 else 45_000_000)
...
dfn = dfn[dfn["execution_payload_gas_limit_adj"].isin([45000000, 60000000])]
...
for i in dfn2.label.unique():
    if dfn2[["label", "execution_payload_gas_limit_adj"]].drop_duplicates().label.tolist().count(i) == 2:
        ...
        if dfn2[dfn2["label"] == i].execution_payload_gas_limit_adj.tolist()[0] == 45000000:
            # Add missing 60M entries with 0 slots
        ...
        elif dfn2[dfn2["label"] == i].execution_payload_gas_limit_adj.tolist()[0] == 60000000:
            # Add missing 45M entries with 0 slots
            for j in dfn2[dfn2["label"] == i].date.unique().tolist():
                dfn2.loc[len(dfn2), ("date"   ,"label"  ,"execution_payload_gas_limit_adj"  ,"slot")] = (
                    j,
                    i,
...
dfn2['gas_limit_45M'] = dfn2['slot'].where(dfn2['execution_payload_gas_limit_adj'] == 45000000, 0)
...
#dfn2 = dfn2.groupby(["date", "label"])["gas_limit_30M", "gas_limit_45M"].sum().reset_index()

dfn2 = dfn2.groupby(["label"])[["gas_limit_45M", "gas_limit_60M"]].sum().reset_index()


# In[ ]:


dfn2["perc"] = dfn2["gas_limit_45M"] / (dfn2["gas_limit_60M"] + dfn2["gas_limit_45M"])
dfn2["gas_limit_60M"] = dfn2["gas_limit_60M"].astype(int)
dfn2["gas_limit_45M"] = dfn2["gas_limit_45M"].astype(int)
dfn2.sort_values(["perc", "gas_limit_60M", "gas_limit_45M"], ascending=False).reset_index(drop=True).to_json("entity_data.json", orient="values")

# In[ ]:


mean_gas_limit = df[df["slot"] >= df.slot.max()-7200].execution_payload_gas_limit.mean()
latest_gas_limit = df.loc[df["slot"].idxmax(), "execution_payload_gas_limit"]
...
if current_limit != 60_000_000:
    df.replace(45_000_000, current_limit, inplace=True)
...
<div class="header-text">
    ...
    Avg. Gas Limit (24h): {mean_gas_limit:,.0f} | Latest: {latest_gas_limit:,.0f}<br/>
